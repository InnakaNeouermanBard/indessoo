...
class LoginRequest extends FormRequest
{
    ...
    public function authenticate(): void
    {
        ...
        // Cek apakah username berupa email atau NIK
        $credentials = $this->only('username', 'password');
        $isEmail = filter_var($credentials['username'], FILTER_VALIDATE_EMAIL);

        // Tentukan guard berdasarkan input email atau nik
        $guard = $isEmail ? 'web' : 'karyawan';
        $field = $isEmail ? 'email' : 'nik';

        // Ganti 'username' menjadi 'email' atau 'nik'
        $credentials = [
