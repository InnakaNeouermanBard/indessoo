<div x-data="{ open: false, showLogoutConfirm: false, showNotificationModal: false }">
    <div class="flex min-h-screen bg-gray-100">
        ...
        <div :class="{ 'w-64': !open, 'w-16': open }" class="fixed inset-y-0 left-0 bg-blue-800 text-white z-30 transition-all duration-300">
            {{-- Logo --}}
            <div class="flex items-center justify-center h-40 border-b bg-white border-gray-700" style="box-shadow: 4px 0 8px rgba(0, 0, 0, 0.1);">
                ...
                <div class="px-4 sm:px-6 lg:px-8">
                    <div class="flex justify-between items-center h-16">
                        ...
                        <div class="flex items-center ml-auto">
                            <div class="text-sm text-gray-600" id="date-time"></div>

                            {{-- Notifikasi --}}
                            <div class="dropdown dropdown-end">
                                <label tabindex="0" class="btn btn-ghost btn-circle relative" id="notificationBtn" @click="showNotificationModal = true">
                                    <div class="indicator">
                            ...
                            <x-dropdown align="right" width="48">
                                ...
                                <x-slot name="content">
                                    ...
                                    <button @click="showLogoutConfirm = true" class="w-full text-left block px-4 py-2 text-sm leading-5 text-gray-700 hover:bg-gray-100 focus:outline-none focus:bg-gray-100 transition duration-150 ease-in-out">
                                        {{ __('Log Out') }}
                                    </button>
    ...
    {{-- Modal Notifikasi --}}
    <div x-show="showNotificationModal" style="display: none" class="fixed inset-0 overflow-y-auto z-40 flex items-center justify-center"
        x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0"
        x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200"
        x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">

        {{-- Backdrop --}}
        <div class="fixed inset-0 transition-opacity" @click="showNotificationModal = false">
            <div class="absolute inset-0 bg-gray-500 opacity-75"></div>
        </div>

        {{-- Modal --}}
        <div class="bg-white rounded-lg overflow-auto shadow-xl transform transition-all sm:max-w-lg sm:w-full p-6 z-50">
            <div class="sm:flex sm:items-start">
                <div class="mx-auto flex-shrink-0 flex items-center justify-center h-12 w-12 rounded-full bg-blue-100 sm:mx-0 sm:h-10 sm:w-10">
                    <i class="ri-notification-3-line text-lg text-blue-500"></i>
                </div>
                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    ...
                    <div class="mt-2">
                        <p class="text-sm text-gray-500">
                            Anda memiliki {{ $countTukarJadwalToday }} pertukaran jadwal yang belum diproses.
                        </p>
                    </div>
                </div>
            </div>
    ...
    <div x-show="showLogoutConfirm" style="display: none" class="fixed inset-0 overflow-y-auto z-50 flex items-center justify-center"
        x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0"
        x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200"
        x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0">
